...
export const WorkerJobsTab: React.FC<WorkerJobsTabProps> = ({
  ...
  const filteredJobs = jobs.filter((job) => {
    if (activeFilterTab === "ASSIGNED") {
      ...
      if (job.status !== "COMPLETED" && job.status !== "AWAITING_PAYMENT") return false;
    } else if (activeFilterTab === "CANCELLED") {
      if (job.status !== "CANCELLED" && job.status !== "REJECTED") return false;
    } else if (activeFilterTab === "HISTORY") {
      if (job.status !== "COMPLETED" && job.status !== "CANCELLED" && job.status !== "REJECTED" && job.status !== "AWAITING_PAYMENT") return false;
    }
  ...
  const getStatusBadge = (status: string) => {
    switch (status) {
      ...
      case "AWAITING_PAYMENT":
        return (
          <span className="px-2.5 py-1 rounded-full text-[10px] font-bold bg-amber-50 text-amber-900 border border-amber-300 whitespace-nowrap flex items-center gap-1">
      ...
      return (
        <div className={`flex items-center ${isMobile ? "flex-wrap gap-2 w-full pt-1" : "justify-end gap-1.5"}`}>
          ...
          <span className="px-3 py-1.5 rounded-xl bg-amber-50 text-amber-900 border border-amber-300 text-xs font-bold flex items-center gap-1.5 shadow-2xs">
            ...
            <span>OTP Verified • Awaiting Citizen Payment</span>
          </span>
        </div>
      );
    }
